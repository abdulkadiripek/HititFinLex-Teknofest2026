...
DATA_DIR = BASE_DIR / "data" / "classification_v3"
...
def validate_splits(rows_by_split: dict[str, list[dict]]) -> None:
    ...
    for split, rows in rows_by_split.items():
        ...
        ids_by_split[split] = {str(row["id"]) for row in rows}
...
def main() -> None:
    set_seed(SEED)
    rows_by_split = {
        "train": read_jsonl(DATA_DIR / "classification_train.jsonl"),
        "validation": read_jsonl(DATA_DIR / "classification_val.jsonl"),
    ...
    results = {
        "campaign": train_task(
            task_name="campaign_v1",
            label_field="is_campaign",
            rows_by_split=rows_by_split,
            preferred_labels=["HAYIR", "EVET"],
        ),
        "product": train_task(
            task_name="product_v1",
            label_field="product_type",
            rows_by_split=rows_by_split,
        ),
    }
    summary_path = BASE_DIR / "models" / "classifier_training_summary.json"
