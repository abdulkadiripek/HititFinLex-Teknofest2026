    ...
    with httpx.Client(base_url=args.api_url, timeout=timeout) as client:
        ...
        for index, document in enumerate(documents, start=1):
            try:
                ...
                campaign = classification.get("is_campaign") or {}
                campaign_label = str(campaign.get("label") or "")
                campaign_score = float(campaign.get("score") or 0.0)
                classification_decision = str(
                    classification.get("decision") or "UNKNOWN"
                ).upper()
                decision_basis = str(
                    classification.get("decision_basis") or "unknown"
                )
                review_reasons = ",".join(
                    str(reason)
                    for reason in (
                ...
                print(
                    ...
                    f"score={product_score:.4f} "
                    f"campaign={campaign_label}:{campaign_score:.4f} "
                    f"decision={classification_decision} "
                    f"basis={decision_basis} reasons={review_reasons} "
