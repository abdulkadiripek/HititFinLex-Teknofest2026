          ...
          {view === "catalog" && (
            <div className="view-stack">
              <section className="catalog-scope-bar panel">
                ...
                {catalogScope !== "live" && <div className="period-chips" aria-label="Tarih aralığı">{historyPeriodOptions.map((period) => <button className={catalogPeriod === period.value ? "active" : ""} key={period.value} onClick={() => { setCatalogPeriod(period.value); loadCatalog(1, undefined, { catalogPeriod: period.value }); }} type="button">{period.label}</button>)}</div>}
              </section>
          ...
          {view === "assistant" && (
            <div className="view-stack">
              <section className="assistant-mode-bar panel">
                ...
                {assistantScope === "history" && <div className="period-chips featured">{historyPeriodOptions.map((period) => <button className={assistantPeriod === period.value ? "active" : ""} key={period.value} onClick={() => setAssistantPeriod(period.value)} type="button">{period.label}</button>)}</div>}
              </section>
